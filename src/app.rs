    ...
    loop {
        if mode_enter_command {
            ...
            stdout()
                .execute(Clear(ClearType::CurrentLine))?
                .execute(MoveToColumn(1))?
                .execute(Print("(r) print registers  (f) flags   (return) enter command"))?;
        }
        if let Key(key) = crossterm::event::read()? {
            if mode_enter_command {
                ...
                if key.code == KeyCode::Enter  {
                    mode_enter_command = false;
                    match parse_line_from_str(&command) {
                        Ok(line) => {
                            stdout()
                                .execute(Clear(ClearType::CurrentLine))?
                                .execute(Print(format!("Executing '{command}' {:?}\r\n", line.bytes)))?;
                ...
                if key.code == KeyCode::Char('r') {
                    crossterm::execute!(
                        ...
                        MoveToColumn(1),
                        Print(format!("AH {:02X} AL {:02X} BH {:02X} BL {:02X}\r\n",vm.registers[0],vm.registers[1],vm.registers[2],vm.registers[3])),
                        Print(format!("CH {:02X} CL {:02X} DH {:02X} DL {:02X}\r\n",vm.registers[4],vm.registers[5],vm.registers[6],vm.registers[7])),
                        Print(format!("AX {:04X} BX {:04X} CX {:04X} DX {:04X}\r\n",u16::from_be_bytes([vm.registers[0],vm.registers[1]]),u16::from_be_bytes([vm.registers[2],vm.registers[3]]),u16::from_be_bytes([vm.registers[4],vm.registers[5]]),u16::from_be_bytes([vm.registers[6],vm.registers[7]]))),
                        Print(format!("FLG {:08b} PC {:04X} SP {:04X} FP {:04X}\r\n\n", vm.registers[8], vm.pc, vm.get_sp(), vm.get_fp()))
                    )?;
                }
...
fn calc_diff(old: [u8; 9], new: [u8; 9]) -> String {
    let reg = [("AH", 0, 1),("AL", 1, 1),("BH", 2, 1),("BL", 3, 1),("CH", 4, 1),("CL", 5, 1),("DH", 6, 1),("DL", 7, 1),("AX", 0, 2),("BX", 2, 2),("CX", 4, 2),("DX", 6, 2),("FLG", 8, 1)];
...
fn decode_flags(byte: u8) -> String {
    ...
    let regs = [("CARRY", CARRY),("OVERFLOW", OVERFLOW),("LESS THAN",LESS_THAN),("GREATER THAN",GREATER_THAN),("ZERO",ZERO),("SIGNED",SIGNED),("INTERRUPTS", INTERRUPTS)];
    let mut output = String::new();
